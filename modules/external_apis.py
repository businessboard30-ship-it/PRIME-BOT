...
def _run_download(url: str, media_type: str) -> Dict:
    ...
    # Ordered fallback chain, not a single format string. A single narrow
    # spec (e.g. "bestaudio[ext=m4a]/bestaudio/best") can raise yt-dlp's
    # "Requested format is not available" even at the metadata-only
    # (download=False) preflight stage — some videos simply don't expose a
    # matching format under the restricted player_client set below (e.g.
    # android/ios sometimes omit the exact codec/ext combo asked for).
    # Each next candidate is progressively looser so a real failure (sign-in
    # required, geo-block, private video) still surfaces immediately instead
    # of silently retrying — see the `except yt_dlp.utils.DownloadError`
    # branch below, which only advances to the next candidate for this one
    # specific "no matching format" case.
    if media_type == "audio":
        ...
        # client. bestvideo+bestaudio asks yt-dlp to download both and
        # merge them with ffmpeg (installed via nixpacks.toml); the widened
        # "bestvideo*+bestaudio/best*" step drops the ext filter for videos
        # where mp4/m4a specifically aren't offered but other containers are.
        format_candidates = [
    ...
    with yt_dlp.YoutubeDL(base_opts) as probe_ydl:
        ...
        # AND so a format-selection failure is caught here — trying each
        # candidate in order — before any bytes are downloaded.
        preflight_info = None
        working_format = None
        last_error = None
        for candidate in format_candidates:
            probe_ydl.params['format'] = candidate
            try:
                preflight_info = probe_ydl.extract_info(url, download=False)
                working_format = candidate
                break
                ...
                if "requested format is not available" in str(e).lower():
                    ...
                    continue
                # Any other DownloadError (sign-in required, geo-block,
                # unsupported URL, private/deleted video) won't be fixed by
                # a looser format string — stop retrying and surface it.
                break
            except Exception as e:
                last_error = e
                break
        ...
        if working_format is None:
            error_text = str(last_error) if last_error else "no matching format found"
            logger.error(f"[v0] yt-dlp preflight failed for {url}: {error_text}")
            hint = ""
            if "cookiefile" not in base_opts and (
    ...
    ydl_opts['format'] = working_format
