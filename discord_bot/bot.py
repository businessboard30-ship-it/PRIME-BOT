...
class AnimeBotDiscord(commands.Bot):
    ...
    async def _best_effort_invite(guild: discord.Guild) -> Optional[str]:
        """Tries, in order: guild's vanity URL (if it has one and the bot
        can see it), an existing invite the bot can read, or creating a
        fresh one on the first channel that allows it. Returns None rather
        than raising if the bot lacks Create Instant Invite everywhere —
        callers must treat a missing invite as normal, not an error, since
        plenty of servers don't grant that permission."""
        try:
        ...
        for channel in guild.text_channels:
            ...
            if perms.create_instant_invite:
                try:
                    invite = await channel.create_invite(max_age=0, reason="Registry entry for /admin guilds")
                    return invite.url
                except (discord.HTTPException, discord.Forbidden):
                    continue
        return None
