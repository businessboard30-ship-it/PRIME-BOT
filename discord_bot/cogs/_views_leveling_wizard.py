...
def render_status_lines(config: dict, role_rows: list) -> list:
    ...
    return [
        ...
        f"{'✅' if role_rows else '⬜'} **Step 3: Role rewards** — {len(role_rows)} configured" if role_rows else "⬜ **Step 3: Role rewards** — none configured",
    ]
...
def build_wizard_view(guild_id: int, clone_id, invoker_id, config: dict, role_rows: list) -> discord.ui.LayoutView:
    ...
    items = [text, discord.ui.Separator(), rate_row, announce_row]
...
DYNAMIC_ITEMS = (
    LevelingXpRateSelect, LevelingAnnounceChannelSelect,
