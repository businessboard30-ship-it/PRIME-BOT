...
class RoastCog(GuildOnlyCog):
    ...
    async def request_from_member(self, interaction: discord.Interaction):
        ...
        if existing:
            await interaction.followup.send(
                f"⚠️ There's already a {existing['status']} roast battle/request in this server.", ephemeral=True,
            )
            return
    ...
    async def manual_trigger(self, interaction: discord.Interaction):
        ...
        if existing:
            await interaction.followup.send(
                f"⚠️ There's already a {existing['status']} roast battle in this server (battle_id={existing['id']}). Quit it first.",
                ephemeral=True,
            )
            return
