...
class JoinDMLayoutView(discord.ui.LayoutView):
    ...
    def __init__(
        ...
        # Support-server link, only on the last page so it doesn't repeat
        # across every page of the wizard. Plain link button — no
        # custom_id, so it needs no DynamicItem registration and is
        # unaffected by timeouts or restarts on its own.
        if page == total_pages - 1 and DISCORD_SUPPORT_SERVER_INVITE:
            bottom_children.append(discord.ui.Button(
...
class _AutomodOptionsView(discord.ui.View):
    ...
    async def action_select(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        await db.set_automod_config(
            ...
            action=select.values[0], timeout_minutes=10 if select.values[0] == "timeout" else None,
        )
