...
class CloneAdminCog(commands.Cog):
    ...
    async def registerclone(self, interaction: discord.Interaction, token: str):
        ...
        # Reject re-registering a token that's already running as an active
        # clone — see get_active_discord_clone_by_bot_user_id's docstring in
        # database.py for the double-gateway-connection bug this prevents.
        existing_active = await db.get_active_discord_clone_by_bot_user_id(result["bot_user_id"])
        if existing_active:
            await interaction.followup.send(
                f"⚠️ **{result['bot_username']}** is already registered as an active clone "
                f"(`#{existing_active['clone_id']}`). Run `/myclones` to see it — no need to "
                f"register the same bot token twice.",
                ephemeral=True,
            )
            return
